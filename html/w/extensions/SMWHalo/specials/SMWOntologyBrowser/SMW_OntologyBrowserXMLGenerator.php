...
class SMWOntologyBrowserXMLGenerator {
	...
	public static function encapsulateAsConceptPartition(array & $titles, array & $resourceAttachments, $limit, $partitionNum, $rootLevel = false) {
		...
		foreach($titles as $e) {
			...
			$result = $result."<conceptTreeElement $leaf title_url=\"$titleURLEscaped\" title=\"".$title_esc."\" img=\"concept.gif\" id=\"ID_$id$count\">$gi_issues$definingRulesXML</conceptTreeElement>";
			$count++;
		}
		if ($rootLevel) {
