...
class PageNavigatorPlugin extends Gdn_Plugin {
    ...
    public function DiscussionsController_BeforeDiscussionMeta_Handler($Sender) {
      if ((C('Plugins.PageNavigator.Show_First')) || (C('Plugins.PageNavigator.Show_Last'))) {
        ...
        $itemlast =  $Object->CountCommentWatch;
       //  $dtitle = $Object->Name;
       // $dtitle =  preg_replace('/\s+/','-',$dtitle);
        $dtitle = "x";
        if($lid > 1) {
       if (C('Plugins.PageNavigator.Show_First')) {
  //   echo sprintf(' <a class="pagenavigator" href="discussion/'  . $did  ."/" . $dtitle . "/#Item_1" . '">First</a>');
         echo sprintf(' <a class="pagenavigator" href="index.php?p=/discussion/'  . $did . '/x/p1' . '">First</a>');
     
          }
         if (C('Plugins.PageNavigator.Show_Last')) {         
      //     echo sprintf(' <a class="pagenavigator" href="discussion/'  . $did  ."/" . $dtitle . "/#Item_" . $itemlast . '">Last</a>');  
         // alternate
          echo sprintf(' <a class="pagenavigator"  href="index.php?p=discussion/comment/' . $lid . '#Comment_' . $lid . '">Last</a>');
